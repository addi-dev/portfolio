...
const about = {
  ...
  details: [
    { key: "Status",   value: '<span class="status-dot"></span> Available for work' },
...
const skills = {
  ...
  database: [
    ...
    { name: "Elasticsearch",icon:"🔍", level: 70 },
  ],
  tools: [
...
const projects = [
  {
    ...
    description: "A full-featured project management SaaS with real-time collaboration, Kanban boards, sprint tracking, and analytics dashboard. Supports teams up to 500 members.",
  ...
  },
  {
    ...
    description: "Headless e-commerce platform with AI-powered recommendations, real-time inventory sync, and a 98% Lighthouse score. Handles 50k+ daily transactions.",
  ...
  },
  {
    ...
    description: "High-throughput analytics ingestion API processing 1M+ events/day. Includes auto-aggregation pipelines, custom query DSL, and interactive chart builder.",
  ...
  },
  {
    ...
    description: "Uptime monitoring and alerting platform with incident management, status pages, and on-call scheduling. Sub-second latency global checks from 12 regions.",
  ...
  },
  {
    ...
    description: "Web app for AI-assisted creative workflows—generate, edit, and export design assets at scale. Integrates with multiple AI image models via a unified API.",
  ...
  },
  {
    ...
    description: "Embeddable real-time messaging SDK with thread support, reactions, file uploads, and end-to-end encryption. Drop-in widget or full headless API.",
...
function typeWriter(el, text, speed = 55) {
  let i = 0;
  el.innerHTML = '<span class="typed-accent"></span><span class="cursor-blink">|</span>';
  const typed = el.querySelector(".typed-accent");
  const blink = el.querySelector(".cursor-blink");
  blink.style.cssText = "animation: blink 0.8s step-end infinite; color: var(--accent);";
  ...
  setTimeout(() => {
    const interval = setInterval(() => {
      if (i < text.length) {
        ...
        setTimeout(() => { blink.style.display = "none"; }, 2000);
      }
    }, speed);
  }, 900);
}
...
function renderAbout() {
  document.getElementById("aboutText").innerHTML = about.paragraphs.map(p =>
    `<p>${p}</p>`
  ).join("");

  document.getElementById("aboutCard").innerHTML = about.details.map(d =>
...
function renderSkills() {
  ...
  panels.innerHTML = Object.entries(skills).map(([category, items]) =>
      ...
      <div class="project-body">
        ...
        <div class="project-links">
          <a href="${proj.github}" target="_blank" rel="noopener" class="btn btn-outline">
            <i data-lucide="github" width="14" height="14"></i> GitHub
          </a>
          <a href="${proj.demo}" target="_blank" rel="noopener" class="btn btn-outline">
            <i data-lucide="external-link" width="14" height="14"></i> Live Demo
          </a>
        </div>
      </div>
    </article>`
...
function renderContact() {
  document.getElementById("contactLinks").innerHTML = contactInfo.map(c =>
...
function renderFooter() {
  ...
  document.getElementById("footerSocials").innerHTML = socials.map(s =>
    `<a href="${s.href}" target="_blank" rel="noopener" class="social-btn" aria-label="${s.icon}">
      <i data-lucide="${s.icon}" width="16" height="16"></i>
    </a>`
  ).join("");
}
...
function initReveal() {
  ...
  // Language cards
  document.querySelectorAll(".language-card").forEach(el => observer.observe(el));
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 2. After DOM is populated, initialize icons
  ...
  // 4. Re-create icons after any dynamic render (for elements added later)
